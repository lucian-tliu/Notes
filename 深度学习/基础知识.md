...
<img src=".\assets\error_surface.png" style="zoom:40%;" />
...
<img src=".\assets\gradient_dec.png" style="zoom:50%;" />
...
<img src=".\assets\local.png" style="zoom:20%;" />
...
在下一部分 [优化](.\优化.md) 中，我们会讨论有哪些方法可以改善梯度下降法的这些问题。
...
<img src=".\assets\sigmoid.png" style="zoom:33%;" />
...
<img src=".\assets\sigmoid1.png" style="zoom:33%;" />

再代入计算

<img src=".\assets\sigmoid2.png" style="zoom:33%;" />
...
<img src=".\assets\ReLU.png" style="zoom:33%;" />
...
-   在实践过程中，一般不会一次性更新所有参数，而是将这些参数**分批**(batch) 处理，因此上面的优化过程可以改写为：
	
	<img src=".\assets\batch.png" style="zoom:33%;" />
	每次对一个 batch 的更新称为一次 update，更新完所有 batchs 称为一个 epoch，对于每个epoch，每批数据都会被随机打乱 (shuffle)。
...
-   原因：模型过于简单。一个尺寸有限的模型，通过改变参数，只能形成一个有限大小的函数空间。如果真实的函数距离这个空间太远，则会出现这种模型偏移
    ...
    <img src=".\assets\model_bais.png" style="zoom:50%;" />
...
-   原因：假设已知真实的数据分布（也就是最理想模型应有的样子，但无法直接观测；且由于数据是离散的，所以用虚线表示）以及训练数据（曲线上的部分点）。
    ...
    <img src=".\assets\overfit1.png" style="zoom:33%;" />

    如果我们采取弹性较大的模型，则有可能使得该模型在这些训练数据点损失非常小，那么我们可能会得到以下结果：

    <img src=".\assets\overfit2.png" style="zoom:33%;" />

    由于模型的弹性够大，对于非训练数据上的其他位置，它有着很高的自由变化程度，因而测试数据上的损失就会变得很大了，如下图所示：

    <img src="D:\Notes\深度学习\assets\overfit3.png" style="zoom:33%;" />

-   解决方法：

    1.   使用更多的训练数据：这是一种简单有效的方法

         <img src="D:\Notes\深度学习\assets\overfit4.png" style="zoom:33%;" />
    ...
    2.   **数据增强**(data augmentation)：在训练数据的基础上得到一些稍作修改的副本，从而形成更多的训练数据。
         ...
         <img src="D:\Notes\深度学习\assets\overfit5.png" style="zoom:50%;" />
...
<img src=".\assets\generalguide.png" style="zoom:30%;" />
...
>
>   <img src="D:\Notes\深度学习\assets\50.png" alt="img" style="zoom:33%;" />
>
