...
对于有多个类型（>2）的分类问题，为了避免前面提到的类型之间的关联性的问题，常用的方法是使用一个多维向量来表示一个类别，例如：
$$
...
0
\end{pmatrix},\quad
\hat{y}_3=\begin{pmatrix}
0\\
0\\
1
\end{pmatrix},\quad
$$
因为分类问题中，我们希望模型能够输出样本处于某一个类别的概率，因此一般会在模型的输出后面接一个 SoftMax 函数
$$
\text{SoftMax}(x_i)=\frac{\mathrm{e}^{x_i}}{\sum\limits_{i=1}^{n}\mathrm{e}^{x_i}}
$$
<img src="./assets/image-20260126161144707-1769415105908-1.png" alt="image-20260126161144707" style="zoom:33%;" />

> [!note]
>
> SoftMax 的作用就是通过指数运算，让较大的输入与较小的输入之间的差距变得更大。

此时的 Logistic 回归模型如下图所示

<img src="./assets/image-20260126161352006-1769415233005-3.png" alt="image-20260126161352006" style="zoom:33%;" />

## 3 Limitation of Logistic Regression

#### 3.1 Feature Transformation

前面的 Logistic 回归存在一些天然的限制，例如对于下面的情况两个类的识别

<img src="./assets/image-20260126162502536.png" alt="image-20260126162502536" style="zoom:33%;" />

Logistic 回归得到的分界线一定是一条直线，而在上面的训练集可以明显看到，仅仅一条直线难以将两个类别分开。为了解决这个问题，我们可以做一次特征变换，例如
$$
\begin{aligned}
x_1'&=\sqrt{{x_1}^2+{x_2}^2}\\
x_2'&=\sqrt{(1-x_1)^2+(1-x_2)^2}
\end{aligned}
$$
变换之后，参数在平面上的分布为

<img src="./assets/image-20260126163258871.png" alt="image-20260126163258871" style="zoom:33%;" />

此时一条直线就可以将两个类别分开了。

#### 3.2 Going to Deep

但并不是每一次我们都能找到 feature 的内在规律，并且我们想要的是人工智能，而非人类智能。一种解决的方法是，我们可以在现在的 Losgistic 模型的前面再加入一层 Logistic 回归自动做特征变换

<img src="./assets/image-20260126164003408.png" alt="image-20260126164003408" style="zoom:33%;" />

我们将其中一个 Sigmoid 称为一个 Neuron，不同 layer 之间的 Sigmoid 相互连接就形成了 Neural Network，而这种机器学习的方式就称为 Deep Learning。
